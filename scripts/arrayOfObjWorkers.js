...
const splitter = ((arr) => { // разделитель массива и формирование объектов, которые в дальнейшем становятся работниками
  ...
  for(let i=0; i < arr.length; i += 1){
    ...
    NewArr.push(createWorker(person[0], person[1], person[2], i));
		createCards(NewArr[i]); // создать карточку
  }
  return NewArr;
})(workers);

const array = splitter; // массив из объектов работников 
console.log(array);

const remover = ((array) => {
	let petReg = /h..ster/gi; // если у работника есть hamster - зафиксировать 
	const cloneArray = array.slice(0); // клон массива, но в этом заменяться на null те, у кого есть hamster
...
const arrayWoutPets = remover; // массив работников, где те у кого есть hamster - null
